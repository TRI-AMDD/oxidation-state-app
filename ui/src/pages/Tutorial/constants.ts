...
export const text4 = `Using the above analysis, we can plot the relative likelihood, P(\u03BC<sub>e</sub>, +3), that an element is found at electronic chemical potential \u03BC<sub>e</sub> and in a +3 oxidation
state. The likelihood is 0 to the left of the
boundary between +2 and +3 oxidation states (b<sub>+2/+3</sub>), then it reaches a plateau in the shaded region, and then
drops down back to zero to the right of the boundary between the 3+ and 4+ oxidation states (b<sub>+3/+4</sub>).`;
...
export const text6 = `We can create similar functions for the +2 and +4 oxidation states:`;
...
export const text15 = `We can generalize the above analysis to determine the most likely set of charge-neutral oxidation states for any composition. We calculate a "likelihood score", L(μ<sub>e</sub>), for any charge-neutral combination of oxidation states by taking the product of the relative likelihood functions, like those shown above, for all oxidation states in the material.  There will be one value of μ<sub>e</sub> that maximizes L(μ<sub>e</sub>), and we call this value μ<sub>opt</sub>. In the above plot, the μ<sub>opt</sub> value is indicated by the dashed red line.  The most likely combination of oxidation states is then the one that has the maximum value of L(μ<sub>opt</sub>) while maintaining charge neutrality.`;
